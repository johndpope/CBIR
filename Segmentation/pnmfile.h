...
static image<rgb> *loadOpenCV(const char *name){
  IplImage *frame = cvLoadImage(name);
  image<rgb> *im = new image<rgb>(frame->width, frame->height);
  for (int i = 0; i < frame->width; ++i)
  {
    for (int j = 0; j < frame->height; ++j)
    {
      rgb* ptr = imPtr(im, i, j);
      ptr->r = CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 2);
      ptr->g = CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 1);
      ptr->b = CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 0);
    }
  }
...
static image<rgb> *loadIplImage(IplImage *frame){
  image<rgb> *im = new image<rgb>(frame->width, frame->height);
  for (int i = 0; i < frame->width; ++i)
  {
    for (int j = 0; j < frame->height; ++j)
...
static IplImage* getIplImage(image<rgb> *im){
  IplImage *frame = cvCreateImage(cvSize(im->width(), im->height()), 8, 3);
  for (int i = 0; i < frame->width; ++i)
  {
    for (int j = 0; j < frame->height; ++j)
    {
      rgb* ptr = imPtr(im, i, j);
      CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 2) = ptr->r;
      CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 1) = ptr->g;
      CV_IMAGE_ELEM(frame, uchar, j, (i * 3) + 0) = ptr->b;
    }
  }
  return frame;
}

static Mat getMat(image<rgb> *im){
  Mat frame = Mat(cvSize(im->width(), im->height()), CV_8UC3);
...
static void saveJPG(image<rgb> *im, const string& name){
  Mat frame = getMat(im);
  imwrite(name, frame);
}

static void displayOpenCV(image<rgb> *im, const char *name){
  Mat frame = getMat(im);
  imshow(name, frame);
}
